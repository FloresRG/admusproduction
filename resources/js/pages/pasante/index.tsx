      ...
      <Grid container spacing={3}>
        {items.map((item) => {
          ...
          const mostrarDetalle =
            !detalleLargo || showMoreIds.has(item.id) ? item.detalle : item.detalle?.slice(0, 100) + '...';

          return (
            <Grid item xs={12} md={4} key={item.id}> {/* Modified this line */}
              <Card
                sx={{
              ...
              >
                <CardContent>
                  ...
                  <Divider sx={{ my: 2 }} />
                  <Box> {/* Changed Typography to Box here */}
...
export default function Pasante({ tareas, filters }: Props) {
      ...
      setAllTareas((prevTareas) => {
        ...
        for (const priority in updatedTareas) {
          if (updatedTareas[priority]) {
            ...
            updatedTareas[priority] = updatedTareas[priority]?.map((task) => {
              if (task.id === id) {
  ...
  const filteredTareas = useMemo(() => {
    const filterFunc = (item: Asignacion) =>
      item.tarea.titulo.toLowerCase().includes(search.toLowerCase());
  ...
  const countEstado = (list?: Asignacion[], estado: string) =>
    list?.filter((t) => t.estado === estado).length || 0;
  ...
  const totalCompletadas =
    countEstado(allTareas.alta, 'completada') +
    countEstado(allTareas.media, 'completada') +
    countEstado(allTareas.baja, 'completada');
  const totalPendientes =
    countTotal(allTareas.alta) + countTotal(allTareas.media) + countTotal(allTareas.baja) - totalCompletadas;
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <Box sx={{ px: 2, py: 4 }}>
        ...
        <PrioritySection
          title={PRIORITY_LABELS.alta}
          items={filteredTareas.alta}
          onEdit={setEditTask}
        />
        <PrioritySection
          title={PRIORITY_LABELS.media}
          items={filteredTareas.media}
          onEdit={setEditTask}
        />
        <PrioritySection
          title={PRIORITY_LABELS.baja}
          items={filteredTareas.baja}
          onEdit={setEditTask}
        />
      </Box>
