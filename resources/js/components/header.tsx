      ...
      >
        <nav className="flex flex-col space-y-4 px-4 py-4">
          ...
          <div className="relative">
            ...
            {showServicesCarousel && (
              <div className="mt-4 overflow-hidden rounded-lg border border-red-500/20 bg-black/60">
                <div className="relative">
                  <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentServiceIndex * 100}%)` }}>
                    {services.map((s, i) => (
                  ...
                  <button onClick={prev} className="absolute top-1/2 left-2 -translate-y-1/2 rounded-full bg-red-600/80 p-2 text-white transition hover:bg-red-600">
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                    </svg>
                  </button>
                  <button onClick={next} className="absolute top-1/2 right-2 -translate-y-1/2 rounded-full bg-red-600/80 p-2 text-white transition hover:bg-red-600">
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="border-t border-white/10 pt-4">
            {auth.user ? (
              <a href={route('dashboard')} className="block rounded-lg bg-red-600/80 py-3 text-center text-white transition hover:bg-red-500">
                Dashboard
              </a>
            ) : (
